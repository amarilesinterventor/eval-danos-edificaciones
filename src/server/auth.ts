 ...
 * externas) — mismo patrón que `wabim-bridges/src/server/auth.ts`.
 *
 * En producción se reemplaza por JWT real (`jsonwebtoken` o `@nestjs/jwt`)
 * firmado con un secreto gestionado de forma segura (variable de entorno /
 * secret manager); la forma de los claims (sub, role, exp) ya es compatible.
 */
import { createHmac, scryptSync, randomBytes, timingSafeEqual } from "node:crypto";

const SECRET = process.env.AUTH_SECRET ?? "edificaciones-dev-secret-change-in-production";
